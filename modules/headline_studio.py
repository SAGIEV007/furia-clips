...
def _terms_nearby(text: str, first: str, second: str, window: int = 96) -> bool:
    """Require two claim terms to occur in the same local passage."""
    for match in re.finditer(re.escape(first), text):
        start = max(0, match.start() - window)
        end = min(len(text), match.end() + window)
        if second in text[start:end]:
            return True
    for match in re.finditer(re.escape(second), text):
        start = max(0, match.start() - window)
        end = min(len(text), match.end() + window)
        if first in text[start:end]:
            return True
    return False
...
def _claim_candidates(text: str, topic: str, speaker_prefix: str = "") -> list[str]:
    folded = normalize(text)
    candidates: list[str] = []
    if "caminho arcaico" in folded and _terms_nearby(folded, "caminho arcaico", "brasil"):
    ...
    if "cobra imposto" in folded and "pais rico" in folded and _terms_nearby(folded, "cobra imposto", "brasil"):
...
def _safe_fake_tweet(text: str, topic: str, mini_context: str) -> list[str]:
    ...
    elif "cobra imposto" in folded and "pais rico" in folded and _terms_nearby(folded, "cobra imposto", "brasil"):
        lead = "O Brasil cobra imposto de país rico, mas precisa encarar a revisão das despesas."
    elif "caminho arcaico" in folded and _terms_nearby(folded, "caminho arcaico", "brasil"):
