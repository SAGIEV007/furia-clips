...
A branch de trabalho conhecida é `manus/rebuild-opus-parity`. A última release funcional de código anterior era a versão `2.2`, commit `074a129`; a versão `2.6` adiciona a primeira ponte funcional Campaign Hub→seeds→expansão→gates→propostas guiadas. As versões documentais `2.4` e `2.5` formalizaram o contrato e o prompt operacional. O Furia ainda não oferece uma experiência diária equivalente ao Garimpo + Campaign Hub, porque o alinhamento com mídia local e o ganho de recall ainda precisam ser medidos e melhorados.

A validação prática da rodada 2.6 deve registrar a suíte completa, as regressões da ponte, a avaliação do payload real do Campaign Hub e a comparação possível com o baseline b354. A release 2.6 não deve reivindicar ganho de recall se a fonte local correspondente não estiver disponível; deve distinguir proposta guiada reconhecida de corte efetivamente recuperado. As revisões 2.3–2.5 foram documentais/operacionais; 2.6 é a primeira alteração funcional Chub→cortes. O modelo oficial pequeno de facetracking continua sendo um asset externo; não o inclua no Git e mantenha o fallback offline quando ele não estiver disponível.
