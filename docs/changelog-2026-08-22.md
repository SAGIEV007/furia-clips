...
## Guarda contra re-renderização concorrente — 2026-08-22
A bancada agora mantém uma guarda thread-safe por `clip_id` durante a janela efetiva do FFmpeg. Se o editor clicar duas vezes em **Renderizar ajuste** para o mesmo clip, a segunda requisição recebe HTTP 409 com uma explicação clara, em vez de iniciar dois processos que poderiam disputar o mesmo arquivo derivado. Clips diferentes continuam independentes, e a guarda é liberada em `finally` mesmo quando o render falha. Foi adicionada regressão para bloqueio por clip e liberação correta. A validação integral deste ciclo confirmou **697 testes aprovados**, JavaScript válido, Python compilado, `git diff --check` e auditoria de segredos sem ocorrência. Campaign Hub permaneceu somente leitura.
## Limites ativos e duração real na reidratação — 2026-08-22
A resposta de projeto agora tenta resolver a fonte original pelas raízes permitidas e medir sua duração com `ffprobe`. Quando a medição é possível, `source_duration` aparece no projeto e em cada clip; quando a fonte não está disponível ou o valor não é finito, o campo permanece `null` e a bancada não inventa a duração do corte como se fosse a duração do vídeo inteiro. Isso permite estender a entrada ou a saída até a duração real da fonte, sem bloquear indevidamente o editor.

Após um ajuste renderizado, a API mantém `start_time`/`end_time` como intervalo canônico original, mas expõe `original_bounds`, `active_bounds`, `active_start`, `active_end`, `active_duration` e `active_render_status`. Para a bancada, `start/end/duration` passam a representar o MP4 ativo que o player reproduz. O card mostra uma nota discreta com a diferença entre intervalo ativo e original canônico. O botão que apenas calcula os limites foi renomeado para **Validar limites**, e o botão de renderização lê diretamente os valores atuais de Entrada e Saída, mesmo que o editor não tenha clicado antes na validação.

Foram adicionadas regressões para fonte disponível, fallback honesto, preservação dos limites canônicos e reidratação do MP4 ajustado. A validação integral deste ciclo confirmou **698 testes aprovados**, JavaScript válido, Python compilado, `git diff --check` e auditoria de segredos sem ocorrência. Campaign Hub permaneceu somente leitura.

## Liberação garantida da guarda em falha de inicialização — 2026-08-22
A proteção contra re-renderizações concorrentes agora envolve também a construção do `VideoCutter`, não apenas a chamada ao FFmpeg. Assim, uma falha de inicialização do renderizador não deixa o clip preso como “em renderização” para sempre; a guarda é liberada em todos os caminhos de exceção. Foi adicionada regressão de integração que simula essa falha e confirma a liberação. A validação integral deste ciclo confirmou **699 testes aprovados**, JavaScript válido, Python compilado, `git diff --check` e auditoria de segredos sem ocorrência. Campaign Hub permaneceu somente leitura.

## Estado visual do re-render na bancada — 2026-08-22
O botão **Renderizar ajuste** agora fica bloqueado imediatamente durante a requisição, recebe `aria-busy` e troca seu rótulo para “Renderizando…”. Cliques duplicados são ignorados no frontend e continuam protegidos pela guarda do backend. O estado é limpo em sucesso, erro ou validação inválida, inclusive quando o card é reconstruído após o render. O botão de validação permanece separado e não sugere playback audiovisual que ainda não foi renderizado. Foi adicionada regressão do contrato visual. A validação integral deste ciclo confirmou **700 testes aprovados**, JavaScript válido, Python compilado, `git diff --check` e auditoria de segredos sem ocorrência. Campaign Hub permaneceu somente leitura.

## Re-render assíncrono e cancelável pela HUD — 2026-08-22
O re-render individual deixou de bloquear a requisição do navegador: a validação continua imediata, mas o FFmpeg e a persistência do MP4 agora rodam em um job persistente `adjust_clip_render`. A HUD mostra a operação, permite usar o botão global de parada e conserva o botão do card ocupado até o estado terminal. O worker usa `ctx.check_cancel()` entre etapas e durante o subprocesso FFmpeg; ao cancelar, o processo é encerrado de forma segura e um arquivo parcial é removido.

A conclusão emite evento específico para atualizar o player, os limites ativos, o cache do MP4 e o estado de revisão. O job grava um artefato mínimo com o `clip_id`, permitindo que a bancada reative o estado ocupado ao reconstruir os cards. O payload segue preservando limites canônicos, duração da fonte e revisão humana. A validação integral deste ciclo confirmou **704 testes aprovados**, JavaScript válido, Python compilado, `git diff --check` e auditoria de segredos sem ocorrência. Campaign Hub permaneceu somente leitura.
