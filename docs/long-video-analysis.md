...
Na aba **Link público**, informe uma URL acessível sem login, preferencialmente um vídeo público do YouTube. O campo aceita tanto `https://www.youtube.com/...` quanto o formato frequentemente copiado do navegador, como `www.youtube.com/...`; nesse segundo caso, o Furia Clips acrescenta `https://` automaticamente antes da validação. O botão **Verificar** obtém metadados e duração quando o extrator disponível consegue fazê-lo. **Importar e transcrever** abre o explorador nativo para você escolher a pasta final, seleciona a melhor combinação de vídeo e áudio até o limite configurado de 1080p e gera automaticamente uma transcrição timestampada. O TXT no formato Tactiq e o JSON estruturado são salvos ao lado do vídeo. Se a transcrição online do Gemini não estiver disponível, o sistema usa Whisper local como fallback; também existe o botão **Gerar do vídeo** na aba Transcrição.
...
O enquadramento automático segue uma política conservadora. Quando há uma única face detectada de forma estável, com confiança e cobertura suficientes, o corte acompanha o locutor em 9:16. Quando há múltiplas faces, troca de câmera, sobreposição ou detector indisponível, o vídeo preserva o aspecto original 16:9 para não cortar a pessoa errada. Cada resultado registra o modo de enquadramento usado (`face_tracking`, `center_crop` ou `original_16_9`).
