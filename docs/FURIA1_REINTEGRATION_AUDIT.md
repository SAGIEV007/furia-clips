...
A suíte dirigida posterior às alterações inclui **33 testes aprovados** nos módulos de perguntas, pool, overlap e raciocínio. A última release gate em diretórios limpos passou com **820 testes aprovados, 27 skips e 2 xfails em 14,71 s**, além de `node --check static/app.js` e `git diff --check`. A suíte inclui transcriber chunked, adapter HTTP, frontend Poolsuite e regressões Furia 1. Nenhum vídeo, JSON privado, SQLite enviado ou workspace temporário será incluído no repositório.
