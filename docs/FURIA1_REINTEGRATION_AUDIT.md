...
Durante a QA de produto, foram reproduzidos e corrigidos cinco problemas que não apareciam apenas na suíte unitária. A fonte ativa era persistida no backend, mas a Mesa retornava ao placeholder ao navegar antes de existirem clips; a restauração automática e o preview da fonte passaram a usar o projeto ativo salvo e a fonte mais recente como fallback. O botão Encontrar cortes iniciava uma nova transcrição mesmo quando havia transcript salvo; agora o backend reutiliza e normaliza essa transcrição, incluindo `segment_count`, texto integral, idioma e proveniência. A Mesa também deixou de calcular uma quantidade negativa de cortes para rever: o adapter expõe `reviewCount` baseado no estado editorial.

O console foi mantido dentro do mesmo Studio, abaixo do conteúdo da revisão, para não cobrir decisões, sliders ou SEO; eventos e o último job são recuperados após recarga, e mensagens duplicadas de conclusão foram removidas. Settings passou a exibir o caminho efetivo do Whisper, o backend de análise, o estado opcional do Gemini sem revelar a chave, e a explicação do Campaign Hub. O espelho agregado oficial `espelho-chub-v1` agora é aceito como memória read-only e apresentado com famílias de hooks, observações e mediana, sem virar score técnico. O ajuste de IN/OUT é preciso por padrão; o alinhamento às bordas do transcript é uma opção explícita e separada.

| Fluxo de QA | Resultado observado | Estado final |
| --- | --- | --- |
| Mesa → Biblioteca/Cortes → Mesa, incluindo recarga | Projeto e vídeo permaneceram selecionados mesmo sem clips; a fonte mais recente é usada como fallback | Corrigido e coberto por regressões de frontend/adapter |
| Settings → Whisper/Gemini/Campaign Hub → salvar | Status real do Whisper visível; Gemini opcional sem chave; preferências salvas localmente; snapshot oficial aceito | Aprovado em QA isolada |
| Transcript salvo → Encontrar cortes | Whisper não foi executado novamente; metadata foi normalizada antes do Furia 1; seleção e render chegaram ao fim | Corrigido e validado com fixture audiovisual |
| Revisão → decisão → headline → IN/OUT → exportação | Aprovação persistiu; headline local atualizou o clip; range invalidou export; MP4 1080×1920 com áudio foi produzido | Aprovado em QA isolada |
| Whisper em fixture sem fala | A falha legítima foi registrada no console como erro, sem botão travado ou silêncio | Comportamento explícito e esperado |

A suíte dirigida posterior às alterações inclui **33 testes aprovados** nos módulos de perguntas, pool, overlap e raciocínio. A release gate final em diretórios limpos passou com **822 testes aprovados, 27 skips e 2 xfails em 14,69 s**, além de `node --check static/app.js` e `git diff --check`. A suíte inclui transcriber chunked, adapter HTTP, frontend Poolsuite, recuperação de jobs e regressões Furia 1. Nenhum vídeo, JSON privado, SQLite enviado ou workspace temporário será incluído no repositório.

Na rodada final, a troca de fonte no mesmo projeto reproduziu uma contaminação de estado: o transcript anterior permanecia associado ao vídeo novo. O adapter passou a invalidar clips, feedbacks, transcrições e identidade técnica da fonte antes de atualizar a mídia, mantendo a memória opcional do Campaign Hub separada; a regressão foi validada por teste HTTP e contra o servidor E2E. Também foi reproduzido um job de exportação interrompido durante um restart. O JobManager agora marca jobs `running` antigos como `stale_recovered`/falha explícita no startup, sem deixar o console preso indefinidamente; jobs enfileirados continuam recuperáveis e o comportamento foi coberto por teste.

A exportação final disparada pelo botão da UI terminou com **8 eventos**, status `completed`, `approvedCount=1`, `reviewCount=0` e `exportedCount=1`. FFprobe confirmou H.264, AAC, 1080×1920 e 38 segundos para o intervalo 9–47; a rota local serviu o mesmo arquivo com HTTP 200. A validação do launcher e do comportamento em Windows real ainda depende de execução em uma máquina Windows.
