...
function renderResultsGrid() {
    ...
    sorted.forEach((clip, i) => {
        ...
        const counterpunch = clip.counterpunch && typeof clip.counterpunch === "object" ? clip.counterpunch : (favorability.counterpunch && typeof favorability.counterpunch === "object" ? favorability.counterpunch : {});
        const counterpunchAvailable = safeBooleanFlag(counterpunch.available);
        const counterpunchReview = safeBooleanFlag(counterpunch.review_required);
        const counterpunchSignal = Number(counterpunch.signal);
        const counterpunchReason = String(counterpunch.reason || "").trim();
...
<span class="material-icons-round">account_tree</span><span><b>Contexto temporal:</b> ${chapterCount > 0 ? `${chapterCount} capítulo(s)${Number.isFinite(chapterScore) ? ` · coerência ${Math.round(Math.max(0, Math.min(100, chapterScore)))}%` : ''}${chapterBridge ? ' · ponte pergunta–resposta preservada' : chapterCount > 1 ? ' · atravessa capítulos; revisar continuidade' : ' · dentro do mesmo bloco'}` : 'fronteira Q&A registrada'}${qaBoundaryBasis ? ` · fronteira: ${escapeHtml(qaBoundaryLabel)}${qaBoundaryReviewRequired ? ' · confirmar locutor' : ''}` : ''}</span></div>` : ''}
                ...
                ${counterpunchAvailable ? `<div class="clip-counterpunch-note ${counterpunchReview ? 'review' : ''}"><span class="material-icons-round">reply</span><span><b>Resposta após possível ataque:</b> ${Number.isFinite(counterpunchSignal) ? `${Math.round(Math.max(0, Math.min(100, counterpunchSignal)))}%` : 'não quantificada'} · ${safeBooleanFlag(counterpunch.hostile_question) ? 'pergunta potencialmente hostil' : 'ponte pergunta–resposta'}${counterpunchReview ? ' · confirmar locutores e contexto' : ''}${counterpunchReason ? ` — ${escapeHtml(counterpunchReason)}` : ''}</span></div>` : ''}
