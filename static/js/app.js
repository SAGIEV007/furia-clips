    ...
    sorted.forEach((clip, i) => {
        ...
        card.innerHTML = `
            ...
            <div class="result-info">
                ...
                ${contextualHookAvailable ? `<div class="clip-hook-provenance ${contextualHookReview ? 'review' : ''}"><span class="material-icons-round">bolt</span><span><b>Hook contextual:</b> ${escapeHtml(String(contextualHook.family || 'não classificado'))} · ${Number(contextualHook.score || 0).toFixed(1)}/100${contextualHook.payoff_confirmed ? ' · payoff próximo' : ' · payoff a confirmar'}${contextualHookReview ? ' · confirmar locutor/áudio no vídeo' : ''}<br><q>${escapeHtml(String(contextualHook.hook_text || ''))}</q></span></div>` : ''}
