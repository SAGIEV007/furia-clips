(() => {
  ...
  function renderChubMemory(chub, compact = false) {
    if (!chub?.available) {
      return `<section class="chub-memory is-empty"><div class="chub-memory-head"><span class="tiny-label">MEMÓRIA DE CAMPANHA</span><span class="chub-status">OPCIONAL</span></div><p><strong>Você pode ignorar esta seção.</strong> Ela só serve para quem já tem um arquivo JSON exportado do Campaign Hub/Garimpo com referências históricas. Não é necessário para importar vídeos, gerar transcrição, encontrar cortes ou exportar.</p><small class="chub-muted">Se você não sabe onde conseguir esse JSON, não conecte nada: o Furia 1 continua funcionando localmente.</small><button class="button button-cyan" data-action="import-chub" title="Importar um arquivo JSON autorizado do Campaign Hub">Tenho um JSON do Chub</button></section>`;
    }
  ...
  function renderClipReviewAlerts(clip) {
    const flags = clip.reviewFlags || clip.review_flags || {};
    const codes = Array.isArray(clip.reviewReasonCodes) ? clip.reviewReasonCodes.slice() : (Array.isArray(clip.review_reason_codes) ? clip.review_reason_codes.slice() : []);
    if (flags.starts_with_question_only && !codes.includes("starts_with_question_only")) codes.push("starts_with_question_only");
    if (flags.ending_interruption && !codes.includes("ending_interruption")) codes.push("ending_interruption");
    const labels = {
      starts_with_question_only: "pergunta sem resposta substancial",
      ending_interruption: "final dentro de intervenção",
      starts_mid_sentence: "início possivelmente no meio da frase",
      starts_with_context_reference: "abertura dependente de contexto",
      payoff_weak_ending: "payoff a confirmar",
      overlap_suspected: "possível sobreposição de falas",
      contains_broadcast_break: "fronteira de intervalo na janela",
      timing_ambiguous: "timing ambíguo",
    };
    const reasons = Array.isArray(clip.reviewReasons) ? clip.reviewReasons : (Array.isArray(clip.review_reasons) ? clip.review_reasons : []);
    const visibleReasons = reasons.length ? reasons.slice(0, 3) : codes.map((code) => labels[code] || String(code).replaceAll("_", " ")).slice(0, 3);
    if (!visibleReasons.length) return "";
    return `<div class="clip-review-risk warning" role="note"><span class="material-icons-round">rule</span><span><b>Revisão editorial:</b> ${visibleReasons.map((reason) => escapeHtml(String(reason))).join(" · ")}<small>O Furia 1 mantém o corte disponível; confirme no vídeo antes de publicar.</small></span></div>`;
  }

  function clipCard(clip) {
    return `<article class="clip-card" data-clip-id="${clip.id}"><div class="clip-thumb">${clip.thumbnail ? `<img loading="lazy" decoding="async" src="${clip.thumbnail}" alt="">` : "<div class=clip-fallback>LOCAL / CUT</div>"}<span class="score-badge">${clip.score}</span></div><div class="clip-content"><div class="clip-card-top"><span>${clipStatusLabel(clip.status)}</span><span>${formatDuration(clip.duration)}</span></div><h3>${escapeHtml(clip.title)}</h3><div class="clip-time">${formatTime(clip.start)} — ${formatTime(clip.end)}</div><div class="clip-signal" aria-label="Sinal local ${clip.score} de 100"><span>SINAL</span><b>${Array.from({ length: 5 }, (_, index) => `<i class="${index < Math.max(1, Math.round(Number(clip.score || 0) / 20)) ? "is-on" : ""}"></i>`).join("")}</b></div><div class="reason-list">${(clip.reasons || []).map((reason) => `<span>${escapeHtml(reason)}</span>`).join("")}</div>${renderClipReviewAlerts(clip)}${renderEditorialBlock(clip.editorialBlock, true)}</div><div class="clip-actions"><button class="clip-open" data-action="open-review" data-clip-id="${clip.id}">Revisar <span>→</span></button><button class="small-decision approve" data-action="decision" data-decision="approved" data-clip-id="${clip.id}" title="Aprovar">✓</button><button class="small-decision reject" data-action="decision" data-decision="rejected" data-clip-id="${clip.id}" title="Rejeitar">×</button></div></article>`;
  }
  ...
  function renderReviewScreen() {
    ...
    attachDynamicActions(target);
    if (project && state.activeClipId) loadDisagreementMatrix(project.id, state.activeClipId);
  }

  function renderDisagreementMatrix(payload, clipId) {
    const host = $("#disagreementMatrix");
    if (!host) return;
    const summary = payload?.summary || {};
    const records = Array.isArray(payload?.records) ? payload.records : [];
    const current = records.slice().reverse().find((item) => String(item?.clip?.clip_id) === String(clipId));
    const currentHuman = current?.human || {};
    const decisions = Object.entries(summary.decision_counts || {}).map(([key, value]) => `${escapeHtml(key)} ${escapeHtml(value)}`).join(" · ");
    const reasons = Object.entries(summary.reason_counts || {}).slice(0, 3).map(([key, value]) => `${escapeHtml(key)} ${escapeHtml(value)}`).join(" · ");
    const discordances = Object.entries(summary.discordance_counts || {}).filter(([key]) => key !== "no_warning").map(([key, value]) => `${escapeHtml(key.replaceAll("_", " "))} ${escapeHtml(value)}`).join(" · ");
    const currentLabel = currentHuman.decision ? `Este corte: ${escapeHtml(currentHuman.decision)}${currentHuman.reason_code ? ` · ${escapeHtml(currentHuman.reason_code)}` : ""}` : "Este corte ainda não tem registro na matriz.";
    host.innerHTML = `<div class="disagreement-head"><span class="tiny-label">MATRIZ DE DISCORDÂNCIA</span><span class="disagreement-status">DESCRITIVA</span></div><p>${summary.count ? `${summary.count} decisão(ões) registradas localmente. Nenhuma delas altera o score.` : "As decisões humanas desta sessão aparecerão aqui."}</p><div class="disagreement-current">${currentLabel}</div>${decisions ? `<small>Decisões: ${decisions}</small>` : ""}${reasons ? `<small>Motivos mais usados: ${reasons}</small>` : ""}${discordances ? `<small>Com alerta automático: ${discordances}</small>` : ""}`;
  }

  async function loadDisagreementMatrix(projectId, clipId) {
    const host = $("#disagreementMatrix");
    if (!host || !projectId) return;
    host.innerHTML = `<span class="tiny-label">MATRIZ DE DISCORDÂNCIA</span><p>Carregando histórico local…</p>`;
    try {
      const payload = await api(`/api/editorial/disagreements?project_id=${encodeURIComponent(projectId)}&limit=200`);
      renderDisagreementMatrix(payload, clipId);
    } catch (error) {
      host.innerHTML = `<span class="tiny-label">MATRIZ DE DISCORDÂNCIA</span><p>Histórico indisponível nesta sessão: ${escapeHtml(error.message)}</p>`;
    }
  }
  ...
  async function analyzeActiveProject(forceReanalysis = false) {
    ...
    finally { state.busy = false; state.busyAction = ""; renderProjectScreen(); renderConsole(); scheduleQueuePoll(0); }
  }
  ...
  async function openSettings() {
    ...
    try {
      ...
      $("#settingGeminiManualVideoAnalysis").checked = Boolean(settings.gemini_manual_video_analysis);
  ...
  async function saveSettings() {
    try {
      const payload = { transcription_source: $("#settingTranscriptionSource").value, ai_backend: $("#settingAiBackend").value, gemini_model: $("#settingGeminiModel").value.trim() || "gemini-2.5-flash", gemini_manual_video_analysis: Boolean($("#settingGeminiManualVideoAnalysis").checked), cut_duration: Number($("#settingCutDuration").value) || 45, subtitle_style: $("#settingSubtitleStyle").value, channel_context: $("#settingChannelContext").value };
